...
__global__ void ray_trace_read_only(uchar4* image, const Sphere* __restrict__ d_s){ 
	//__restrict__ asserts to the compiler that the pointers are not aliased, may have to qualify image as well
	int x = threadIdx.x + blockIdx.x * blockDim.x;
	int y = threadIdx.y + blockIdx.y * blockDim.y;
	int offset = x + y * blockDim.x * gridDim.x;
	float   ox = (x - IMAGE_DIM / 2.0f);
	...
	float   maxz = -INF;
	for (int i = 0; i<d_sphere_count; i++) {
		const Sphere *s = &d_s[i]; //__ldg(&d_s[i]);
		float   n;
